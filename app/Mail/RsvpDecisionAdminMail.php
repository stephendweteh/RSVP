...
class RsvpDecisionAdminMail extends Mailable
{
    ...
    private function rendered(): array
    {
        return $this->renderedCache ??= (function (): array {
            $vars = MailTemplateRenderer::varsForDecisionAdmin($this->rsvp, $this->decision);

            return MailTemplateRenderer::render(
                MailTemplate::query()->where('slug', MailTemplate::SLUG_RSVP_DECISION_ADMIN)->firstOrFail(),
                $vars['html'],
                $vars['text'],
            );
        })();
